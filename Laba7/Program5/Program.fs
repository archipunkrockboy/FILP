...
let MaxFrequencyElemIndexes list = 
    //(fst(List.maxBy(fun x->snd(x)) (List.countBy (fun x->x) list)))//находит наиболее часто встречающийся элемент списка   
    List.filter (fun x->x<>(-1)) (List.mapi (fun i x -> if x = ((fst(List.maxBy(fun x->snd(x)) (List.countBy (fun x->x) list)))) then i else -1 ) list)
